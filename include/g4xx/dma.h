...
/// Trait carrier for DMAMUX — list this in `PeripheralEnabler` whenever the
/// firmware uses a DMA channel with peripherally-triggered requests (G4 only).
struct Dmamux
{
	using RccTrait_ = Clocks::RccTrait<
		Clocks::RccBit<Clocks::RccReg::kAhb1En,
