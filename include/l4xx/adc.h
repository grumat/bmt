...
/*
CMSIS header (stm32l4xx.h) must be included before this file.
Provides: ADC_TypeDef, ADC_Common_TypeDef, ADC1/2/3_BASE,
ADC_CR_ADEN/ADSTART/ADVREGEN/ADCAL/ADDIS,
ADC_CFGR_DMAEN/CONT/RES/ALIGN/EXTEN/EXTSEL/OVRMOD/AUTDLY,
ADC_CFGR2_OVSE/OVSR/OVSS,
ADC_ISR_EOC/ADRDY,
ADC_SMPR1/2_*, ADC_SQR1/2/3/4_*,
RCC_AHB2ENR_ADCEN, RCC_AHB2RSTR_ADCRST,
ADC1/2/3 pointer macros, ADC1_COMMON_BASE.
*/
...
{
	// All ADCs share a single RCC enable / reset bit on L4.
...
struct Chan
{
	...
	static_assert(kChan <= 18, "L4xx ADC channels are 0..18");
};
...
/// L4 layout — 4 registers, 5-bit SQ field, 6-bit stride (gap at bit 5):
...
/// that RccEnabler toggles (AHB2ENR.ADCEN only gates the register interface).
/// After reset CKMODE = kAsync and the RCC mux (CCIPR.ADCSEL) selects no clock,
/// so the analog domain is unclocked until one of these is configured.
///   kAsync     — use the RCC-muxed async kernel clock; CCIPR.ADCSEL must then
...
struct AnySequence
{
	...
	// NOTE: L4 swaps SMPR1/SMPR2 relative to F1.
...
{
	// ── F1xx compat fields ──────────────────────────────────────────────
	static constexpr bool    kScan        = false;	// no-op (implicit on L4)
	...
	static constexpr uint8_t kExtTrigSel  = 0;		// CFGR.EXTSEL[3:0]
	...
	// defer to the RCC clock tree (CCIPR.ADCSEL).
	static constexpr CkMode kCkMode      = CkMode::kHclkDiv4;

	// ── Oversampling (CFGR2) ────────────────────────────────────────────
	static constexpr bool    kOvs        = false;	// OVSE
...
struct AnySetup
{
	...
	ALWAYS_INLINE static void Init()
	{
		...
		// the async mux (CCIPR.ADCSEL) selects no clock. kAsync leaves CKMODE
		// untouched and trusts the clock tree to have set up ADCSEL.
